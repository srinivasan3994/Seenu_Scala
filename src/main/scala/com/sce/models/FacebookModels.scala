...
case class FacebookSimpleMsg(text: String)

case class FacebookSimpleMsgTemplate(recipient: FacebookRecipient,message: FacebookSimpleMsg)
...
trait FacebookJsonSupport extends DefaultJsonProtocol with SprayJsonSupport {
  ...
  // simple message
  implicit val facebookSimpleMsgJsonFormat = jsonFormat1(FacebookSimpleMsg)
  implicit val facebookSimpleMsgTemplateJsonFormat = jsonFormat2(FacebookSimpleMsgTemplate)
